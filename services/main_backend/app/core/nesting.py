...
def _copy_entities_to_msp(
    ...
    offset_y: float,
    rotation_deg: float,
    part_min_x: float = 0.0,
    part_min_y: float = 0.0,
):
    """
    Directly copy all modelspace entities from source DXF into out_msp,
    applying the given XY translation and rotation.
    ...
    for entity in src_msp:
        try:
            ...
            if etype == "LINE":
                s = entity.dxf.start
                e = entity.dxf.end
                out_msp.add_line(transform_pt(s.x, s.y), transform_pt(e.x, e.y),
                ...
                if len(pts) >= 2:
                    out_msp.add_lwpolyline(pts, close=closed,
                                           dxfattribs={"layer": entity.dxf.get("layer", "0")})

            elif etype == "CIRCLE":
                c = entity.dxf.center
                out_msp.add_circle(transform_pt(c.x, c.y), radius=entity.dxf.radius,
                                   dxfattribs={"layer": entity.dxf.get("layer", "0")})

            elif etype == "ARC":
                c = entity.dxf.center
                out_msp.add_arc(transform_pt(c.x, c.y), radius=entity.dxf.radius,
                ...
                out_msp.add_ellipse(transform_pt(c.x, c.y), major_axis=(rma_x, rma_y),
                ...
                spline = out_msp.add_spline(dxfattribs={"layer": entity.dxf.get("layer", "0")})
...
def _create_default_dxf(path: str):
    """Auto-creates a sample 2D DXF profile if file path does not exist on disk."""
    try:
        ...
        if "flange" in filename:
            w, h = 180.0, 180.0
            msp.add_circle((0, 0), radius=90.0, dxfattribs={"layer": "OUTLINE"})
            msp.add_circle((0, 0), radius=35.0, dxfattribs={"layer": "HOLES"})
            for k in range(6):
                ang = k * (3.14159 * 2 / 6)
                msp.add_circle((65.0 * np.cos(ang), 65.0 * np.sin(ang)), radius=6.0, dxfattribs={"layer": "HOLES"})
        else:
            w, h = 150.0, 100.0
            half_w, half_h = w / 2.0, h / 2.0
            msp.add_lwpolyline([(-half_w, -half_h), (half_w, -half_h), (half_w, half_h), (-half_w, half_h), (-half_w, -half_h)], dxfattribs={"layer": "OUTLINE"})
            for hx, hy in [(-half_w + 15, -half_h + 15), (half_w - 15, -half_h + 15), (-half_w + 15, half_h - 15), (half_w - 15, half_h - 15)]:
                msp.add_circle((hx, hy), radius=5.0, dxfattribs={"layer": "HOLES"})
        doc.saveas(abs_path)
    except Exception as e:
        print(f"Failed to auto-create DXF for {path}: {e}")

# Import numpy for math functions in _create_default_dxf
import numpy as np
...
def nest_parts(
    ...
    for part in normalized_parts:
        if not os.path.isfile(part.path):
            # Auto-create missing sample DXF file
            _create_default_dxf(part.path)
    ...
    for idx, g_sheet in enumerate(generated_sheets, start=1):
        ...
        for p in placements:
            part_path = p["part"]
            # Find the matching NestablePart object for min_x/min_y
            matching_part = next(
                (np for np in normalized_parts if str(np.path) == str(part_path)), None
            )
            min_x = matching_part.min_x if matching_part else 0.0
            min_y = matching_part.min_y if matching_part else 0.0
