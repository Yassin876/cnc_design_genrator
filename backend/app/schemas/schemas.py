...
class JobProgressEvent(BaseModel):
    ...
    dxf_content: Optional[str] = None  # For 2D external API responses
    dxf_url: Optional[str] = None  # For 2D external API responses
...
class Edit3DRequest(BaseModel):
    user_id: Optional[str] = "default_user"
    project_id: Optional[str] = None
    file_path: str  # Existing STL/GLB file to edit
...
class StockSheetSchema(BaseModel):
    ...
    quantity: int = 5
    material: Optional[str] = "Plywood"

class NestingRequest(BaseModel):
    part_paths: List[Any]
    sheet_width: Optional[float] = 1200.0
    sheet_height: Optional[float] = 600.0
    sheet_thickness: Optional[float] = 18.0
