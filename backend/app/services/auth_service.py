...
from backend.app.core.mail import send_otp_email
from backend.app.core.exceptions import AuthException
from backend.app.utils.validators import normalize_email, validate_password_strength
...
class AuthService:
    ...
    def verify_otp(db: Session, req: VerifyOTPRequest):
        ...
        if req.purpose == "registration":
            ...
            # Code is valid! Do not mark as used yet; reset_password will consume it when changing the password
    ...
    def resend_otp(db: Session, req: ResendOTPRequest) -> GenericAuthResponse:
        ...
        sent = send_otp_email(recipient_email=email, recipient_name=user.name, otp_code=otp_code, purpose=req.purpose)
        if not sent:
            if not settings.SMTP_USERNAME or not settings.SMTP_PASSWORD:
                raise AuthException(
                    500,
                    "فشل إرسال البريد الإلكتروني: بيانات SMTP غير مجهزة في ملف .env.",
                    "AUTH_SMTP_NOT_CONFIGURED"
                )
            else:
                raise AuthException(
                    500,
                    "فشل إرسال البريد الإلكتروني عبر خادم SMTP. يرجى التحقق من صحة بيانات الاتصال.",
                    "AUTH_SMTP_DELIVERY_FAILED"
                )
    ...
    def login_user(db: Session, req: LoginRequest) -> TokenResponse:
        ...
        if user.google_id and not user.password_hash:
            raise AuthException(401, "Please sign in with Google.", "AUTH_GOOGLE_ONLY_ACCOUNT")
    ...
    def forgot_password(db: Session, req: ForgotPasswordRequest) -> GenericAuthResponse:
        ...
        if user and user.is_active:
            ...
            sent = send_otp_email(recipient_email=email, recipient_name=user.name, otp_code=otp_code, purpose="password_reset")
            if not sent:
                if not settings.SMTP_USERNAME or not settings.SMTP_PASSWORD:
                    raise AuthException(
                        500,
                        "فشل إرسال البريد الإلكتروني: بيانات SMTP غير مجهزة في ملف .env (تأكد من ضبط SMTP_USERNAME و SMTP_PASSWORD).",
                        "AUTH_SMTP_NOT_CONFIGURED"
                    )
                else:
                    raise AuthException(
    ...
    @staticmethod
    def update_user_profile(db: Session, user_id: str, name: str = None, avatar_url: str = None) -> UserRead:
        user = UserRepository.update_profile(db, user_id, name=name, avatar_url=avatar_url)
        if not user:
    ...
    def change_password(db: Session, user_id: str, current_password: str, new_password: str, confirm_password: str = None) -> GenericAuthResponse:
        ...
        # Prevent Google-only accounts from changing password
        if user.google_id and not user.password_hash:
            raise AuthException(400, "Google-only accounts cannot change password. Please use Google sign-in.", "AUTH_GOOGLE_ONLY_ACCOUNT")
    ...
    def request_email_change(db: Session, user_id: str, current_password: str, new_email: str) -> GenericAuthResponse:
        ...
        # Prevent Google-only accounts from changing email with password
        if user.google_id and not user.password_hash:
            raise AuthException(400, "Google-only accounts cannot change email. Please use Google sign-in.", "AUTH_GOOGLE_ONLY_ACCOUNT")
