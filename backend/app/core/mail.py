...
def send_otp_email(recipient_email: str, recipient_name: str, otp_code: str, purpose: str = "registration") -> bool:
    ...
    is_placeholder = (
        ...
        or "your_email" in settings.SMTP_USERNAME
        or "your_gmail" in settings.SMTP_PASSWORD
    )

    if is_placeholder:
        request_logger.warning(
            f"SMTP credentials are not configured or still have default placeholders in .env. "
            f"Email delivery simulated for {recipient_email}. "
    ...
    try:
        ...
        error_logger.error(f"SMTP Authentication Error sending to {recipient_email}: Please check SMTP_USERNAME and SMTP_PASSWORD in .env. Details: {e}")
        if settings.ENVIRONMENT == "development":
            request_logger.info(f"[DEV MODE FALLBACK] Using logged OTP code for {recipient_email}: {otp_code}")
            return True
        return False
