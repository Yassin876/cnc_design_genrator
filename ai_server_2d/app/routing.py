...
async def route_and_generate(image_path: str, output_path: str, prompt: str = None, params: dict = None) -> str:
    ...
    if False and img_type == "CAD":
        ...
        try:
            ...
            log.warning("HF Space fallback trigger: %s", err)
            import cv2
            import numpy as np
        ...
        log.info("Saved HF mask to: %s", saved_mask_path)
        ...
        if doc is None:
            ...
            raise HTTPException(
                status_code=422,
                detail=(
                    "Potrace vectorization failed to find contours in the mask. "
                    "Use a high-contrast silhouette with a clear dark shape on a light background."
                ),
            )
